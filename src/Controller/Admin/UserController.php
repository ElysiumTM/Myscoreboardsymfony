...
class UserController extends AbstractController
{
    ...
    Si il n'y a pas de player relié, créez un nouveau player (son nickname sera égal au pseudo de l'user, l'email sera pseudo@yopmail.com), relier ce player au user, et enregistrez les modifications en bdd */
    #[Route('/update', name: 'app_admin_user_update', methods: ['GET'])]
    public function update(UserRepository $ur, EntityManagerInterface $em): Response
    {
        $liste = $ur->findAll();
        $compteur = 0;
        foreach ($liste as $user) {
            if( in_array("ROLE_ADMIN", $user->getRoles()) && !$user->getPlayer() ) {
                    ...
                    $player->setEmail( $user->getPseudo() . '@yopmail.com');
